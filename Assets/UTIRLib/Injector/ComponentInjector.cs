    ...
    public static class ComponentInjector
    {
        ...
        public static void Inject(Component sourceComponent)
        {
            ...
            if (TryGetMarkedMembers(componentType, out MemberInfo[] markedMembers))
            {
                ...
                {
                    targetComponent = GetTargetComponent(sourceComponent, markedMember);

                    if (targetComponent.IsNotNull())
                    {
                        SetComponent(sourceComponent, markedMember, targetComponent);
                    }
                }
            }
        }

        private static object? GetTargetComponent(Component sourceComponent, MemberInfo member)
        {
            ...
            if (!CheckMemberValue(sourceComponent, sourceComponent.GetTypeName(), member.Name, memberValue, settings))
            {
                return null;
            }
            ...
            {
                targetComponent = GetTargetAssignedObject(sourceComponent, targetComponentType, getComponentAttribute);
            }
            else
            {
                targetComponent = GetComponentFromSource(sourceComponent, targetComponentType, getComponentAttribute);
            }
        ...
        private static bool IsOptionalMember(MemberInfo member) => member.IsDefined(typeof(OptionalAttribute));
        ...
        private static Component? GetComponentFromSource(Component sourceComponent, Type targetComponentType,
            ...
            switch (attribute)
            {
                case GetComponentInChildrenAttribute attributeChild:
                    if (attributeChild.HasChildName &&
                        sourceComponent.transform.TryFind(attributeChild.ChildName!, out Transform? child))
                    {
                ...
                case GetComponentInParentAttribute attributeParent:
                    if (attributeParent.HasParentName &&
                        sourceComponent.transform.TryFind(attributeParent.ParentName!, out Transform? parent))
                    {
                ...
                default: return sourceComponent.GetComponent(targetComponentType);
            }
        }

        private static object? GetTargetAssignedObject(Component sourceComponent, Type targetComponentType,
            GetComponentAttribute attribute)
        {
            switch (attribute)
            {
        ...
        private static void SetComponent(Component sourceComponent, MemberInfo member, object targetComponent)
        {
            switch (member)
            {
                case FieldInfo field:
        ...
        {
            MemberInfo[] members = type.GetFields(BindingFlagsDefault.InstanceAll.ToBindingFlags()).Cast<MemberInfo>().
                Concat(type.GetProperties(BindingFlagsDefault.InstanceAll.ToBindingFlags()).Cast<MemberInfo>()).ToArray();
