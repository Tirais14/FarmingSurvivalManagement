...
namespace Zenject.Internal
{
    ...
    public class DecoratorProvider<TContract> : IDecoratorProvider
    {
        ...
        List<IZenFactory<TContract, TContract>> _decoratorFactories;
        ...
        void LazyInitializeDecoratorFactories()
        {
            ...
            {
                _decoratorFactories = new List<IZenFactory<TContract, TContract>>();

                for (int i = 0; i < _factoryBindIds.Count; i++)
                {
                    var bindId = _factoryBindIds[i];
                    var factory = _container.ResolveId<IZenFactory<TContract, TContract>>(bindId);
